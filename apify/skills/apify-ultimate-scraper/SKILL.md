...
**Rule: pass `--user-agent apify-github-copilot-plugin/apify-ultimate-scraper` only on actor start and actor run commands (`apify actors start`, `apify actors call`). Do not add it to login, info, search, or other CLI commands.**
...
```bash
    apify actors start "ACTOR_ID" -i 'JSON_INPUT' --user-agent apify-github-copilot-plugin/apify-ultimate-scraper --json 2>/dev/null
```
...
```bash
    apify actors call "ACTOR_ID" -i 'JSON_INPUT' --user-agent apify-github-copilot-plugin/apify-ultimate-scraper --json 2>/dev/null
```
